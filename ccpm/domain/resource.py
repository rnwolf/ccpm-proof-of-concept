...
class Resource:
    ...
    def __init__(
        ...
        Raises:
            ValueError: If id is None or empty, name is None or empty, or capacity is negative
            TypeError: If capacity is not a number
        """
        # Validate id
        if id is None or str(id).strip() == "":
            raise ValueError("Resource id cannot be None or empty")

        # Validate name
        if name is None or str(name).strip() == "":
            raise ValueError("Resource name cannot be None or empty")

        # Validate capacity
        if not isinstance(capacity, (int, float)):
            raise TypeError("Resource capacity must be a number")
        ...
        if calendar is not None:
            if not isinstance(calendar, dict):
                raise TypeError(
                    "Calendar must be a dictionary mapping date strings to capacity values"
                )

            # Validate each date and capacity in the calendar
            for date_str, date_capacity in calendar.items():
                if not isinstance(date_str, str):
                    raise TypeError(
                        f"Calendar date key must be a string, got {type(date_str).__name__}"
                    )
                ...
                try:
                    ...
                    raise ValueError(
                        f"Calendar date '{date_str}' is not in the format 'YYYY-MM-DD'"
                    )

                if not isinstance(date_capacity, (int, float)):
                    raise TypeError(
                        f"Calendar capacity for date {date_str} must be a number"
                    )

                if date_capacity < 0:
                    raise ValueError(
                        f"Calendar capacity for date {date_str} cannot be negative"
                    )
        ...
        self.allow_overallocation = bool(allow_overallocation)
        self.tags = tags or []
    ...
    def add_tag(self, tag):
        ...
        Args:
            tag: Tag to add (must be a string)
    ...
    def allocate(self, task_id, date, units=1):
        ...
        Raises:
            TypeError: If date is not a datetime object or units is not a number
            ValueError: If task_id is None, or units is negative or zero
            ResourceOverallocationError: If allocation would exceed capacity and overallocation is not allowed
        """
        # Input validation
        if task_id is None:
        ...
        if units > available and not self.allow_overallocation:
            raise ResourceOverallocationError(
                ...
                f"Only {available} units available. Resource: {self.name}"
            )
        ...
        # Get existing allocations for this date after adding this one
        allocated_capacity = sum(self.allocations[date_str].values())

        # Track over-allocation if applicable
        overallocated_units = max(0, allocated_capacity - total_capacity)
    ...
    def deallocate(self, task_id, date):
        ...
        if date_str in self.allocations and task_id in self.allocations[date_str]:
            ...
            if date_str in self.overallocations:
                ...
                # Get remaining allocations for this date
                remaining_allocations = self.allocations.get(date_str, {})
                allocated_capacity = sum(remaining_allocations.values())

                # Recalculate overallocation
                overallocated_units = max(0, allocated_capacity - total_capacity)

                if overallocated_units > 0:
                    self.overallocations[date_str] = overallocated_units
                else:
                    # Remove overallocation record if no longer overallocated
                    del self.overallocations[date_str]
    ...
    def is_available_for_period(self, start_date, end_date, units=1):
        ...
        Raises:
            ...
            ValueError: If start_date is after end_date or units is negative
        """
    ...
    def add_planned_assignment(self, task_id, start_date, end_date):
        ...
        Raises:
            TypeError: If start_date or end_date is not a datetime object
            ValueError: If task_id is None or start_date is after end_date
        """
        if task_id is None:
            raise ValueError("Task ID cannot be None")
    ...
    def update_planned_assignment(self, task_id, start_date=None, end_date=None):
        ...
        Raises:
            TypeError: If start_date or end_date is not a datetime object when provided
        ...
        if (
            "planned_start" in self.planned_assignments[task_id]
            and "planned_end" in self.planned_assignments[task_id]
        ):
            start = self.planned_assignments[task_id]["planned_start"]
            end = self.planned_assignments[task_id]["planned_end"]

            if start > end:
                raise ValueError(
                    f"Planned start date {start} cannot be after planned end date {end} for task {task_id}"
                )
